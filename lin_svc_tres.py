...
#This function performs t-test feature selection on resampling data combination method along with training LinearSVC under 10-fold cross validation
...
def lin_svc_tres(finC_x, finC_y, finT_x, finT_y):
    ...
    ttc1 = tt.t_test(dataC_x_train, dataC_y_train, 5000)

    ttt1 = tt.t_test(dataT_x_train, dataT_y_train, 5000)

    new_x = pd.concat([ttc1, ttt1], axis = 1)
    ...
    for num in features_num:
        ...
        ttc = tt.t_test(ttc1, dataC_y_train, num)

        ttt = tt.t_test(ttt1, dataT_y_train, num)

        tt_ = tt.t_test(new_x, dataT_y_train, num)

        new_x = pd.concat([tt_, pd.concat([ttc, ttt], axis = 1)], axis = 1)
